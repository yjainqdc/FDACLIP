...
class AlphaCLIP_Video(nn.Module):
    '''
    ...
    def __init__(self, config):
        super(AlphaCLIP_Video, self).__init__()
        self.config = config
        #self.clip = CLIPModel.from_pretrained("/sshfs/jiaao/workdir/CLIP_test/PromptSwitch-main/CLIP_Vit_32/")
        self.clip, self.preprocess = alpha_clip.load("ViT-B/32", device=device)  # change to your own ckpt path
    ...
    def forward(self, data, return_all_frames=False):
        ...
        video_data = video_data.reshape(-1, 3, self.config.input_res, self.config.input_res).float()
        mask_data = mask_data.reshape(-1, 1, self.config.input_res, self.config.input_res).float()
        text_features = self.clip.encode_text(text_data).float()
        video_features = self.clip.visual(video_data, mask_data).float()
