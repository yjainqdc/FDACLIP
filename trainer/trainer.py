...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, data in enumerate(self.train_data_loader):
            ...
            #print(data.keys())
            if self.arch == 'frame_clip':
                data['text'] = clip.tokenize(data['text'])
            elif self.arch == 'alphaclip_video' or self.arch == 'alphaclip_video_transformer':
                data['text'] = alpha_clip.tokenize(data['text'])
                data['diff_mask'] = data['diff_mask'].to(self.device)
            else:
            ...
            if batch_idx % self.log_step == 0:
                if 'caption' in self.loss.keys():
                    ...
                    print('Train Epoch: {} dl: {}/{} Loss Ctr: {:.6f} '.format(
                        epoch,
                        batch_idx,
                        num_steps - 1,
                        loss.detach().item()))
    ...
    def _valid_epoch_step(self, model, epoch, step, num_steps):
        ...
        with torch.no_grad():
            for _, data in tqdm(enumerate(self.valid_data_loader)):
                ...
                if self.arch in ['frame_clip']:
                    vid_embed_arr_pooled.append(vid_embed_pooled.cpu())
            ...
            if self.arch in ['frame_clip']:
                vid_embeds_pooled = torch.cat(vid_embed_arr_pooled)




            if self.arch not in ['frame_clip']:
                ...
                text_embeds_per_video_id, vid_embeds_pooled_per_video_id = generate_embeds_per_video_id(text_embeds,vid_embeds_pooled, all_vid_ids, self.pooling_type_test)
            else:
                # Since we have all pairs, remove duplicate videos when there's multiple captions per video
                vid_embeds_per_video_id = {}
                for idx, v_id in enumerate(all_vid_ids):
                    if v_id not in vid_embeds_per_video_id:
                        vid_embeds_per_video_id[v_id] = vid_embeds_pooled[idx]
                vid_embeds = torch.stack([vid_embeds_per_video_id[v_id] for v_id in vid_embeds_per_video_id])
                text_embeds_per_video_id, vid_embeds_pooled_per_video_id = generate_embeds_per_video_id(text_embeds,vid_embeds, all_vid_ids, self.pooling_type_test)
